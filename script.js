...
const soundHintSkip = document.getElementById('shSkip');
...
function turnOnSongAndDismiss() {
  // Click the audio panel mute button → unmutes (initial state is muted).
  const apMute = document.getElementById('apMute');
  if (apMute) apMute.click();
  dismissSoundHint();
}
...
if (soundHint) {
  ...
  if (soundHintOk) soundHintOk.addEventListener('click', turnOnSongAndDismiss);
  if (soundHintSkip) soundHintSkip.addEventListener('click', dismissSoundHint);
